...
//   const [selectedType, setSelectedType] = useState('all');
//   const [expandedCards, setExpandedCards] = useState({});
//   const [allPromotions, setAllPromotions] = useState(staticPromotions);

//   useEffect(() => {
//     // Load promotions from localStorage and combine with static promotions
//     const savedPromotions = JSON.parse(localStorage.getItem('promotions') || '[]');
//     setAllPromotions([...staticPromotions, ...savedPromotions]);
//   }, []);
...
//       return matchesType && matchesSearch;
//     });
//   }, [searchQuery, selectedType, allPromotions]);
...
//               <span className="ml-2 text-sm text-gray-500 line-through">${promotion.originalPrice}</span>
//             </div>
//           </div>
...
import { Search, Smartphone, Headphones, ChevronDown, ChevronUp, Clock, Tag, Trash2, Edit } from 'lucide-react';
...
const staticPromotions = [
  ...
  // ... (keep all other static promotions)
];
...
function Promotions() {
  ...
  const handleDelete = (id) => {
    // Check if it's a static promotion
    const isStatic = staticPromotions.some(promo => promo.id === id);
    if (isStatic) {
      alert("Cannot delete static promotions!");
      return;
    }

    // Get saved promotions and filter out the deleted one
    const savedPromotions = JSON.parse(localStorage.getItem('promotions') || '[]');
    const updatedPromotions = savedPromotions.filter(promo => promo.id !== id);
    localStorage.setItem('promotions', JSON.stringify(updatedPromotions));
    
    // Update state
    setAllPromotions([...staticPromotions, ...updatedPromotions]);
  };
  ...
  const renderPromotionCard = (promotion) => {
    const isExpanded = expandedCards[promotion.id];
    const daysRemaining = getDaysRemaining(promotion.endDate);
    const isStatic = staticPromotions.some(p => p.id === promotion.id);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className='flex justify-end mr-10 '>
            <button
              ...
              className="flex justify-end bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200 shadow-md"
            >
              {/* <IoAddOutline className="w-5 h-5" /> */}
              <span>Add Promotions</span>
            </button>
      </div>
      <main className="container mx-auto px-4 pb-16 mt-0">
        {filteredPromotions.length === 0 ? (
          renderEmptyState()
        ) : (
          <>
